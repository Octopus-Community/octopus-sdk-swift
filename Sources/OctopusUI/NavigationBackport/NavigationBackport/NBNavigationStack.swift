...
public struct NBNavigationStack<Root: View, Data: Hashable>: View {
  ...
  @Compat.StateObject var path: NavigationPathHolder
  @Compat.StateObject var destinationBuilder = DestinationBuilderHolder()
  @Compat.StateObject var navigator: Navigator<Data> = .init(.constant([]))
  ...
  public var body: some View {
    content
      ...
      .onValueChanged(of: externalTypedPath) { externalTypedPath in
        guard isUsingNavigationView else {
          return
        }
      ...
      .onValueChanged(of: internalTypedPath) { internalTypedPath in
        guard isUsingNavigationView else {
          return
        }
      ...
      .onValueChanged(of: path.path) { path in
        if useInternalTypedPath {
  ...
  public init(path: Binding<[Data]>?, @ViewBuilder root: () -> Root) {
    ...
    self.root = root()
    _path = Compat.StateObject(wrappedValue: NavigationPathHolder(path: path?.wrappedValue ?? []))
    useInternalTypedPath = path == nil

    let navigator = useInternalTypedPath ? Navigator(.constant([])) : Navigator($externalTypedPath)
    _navigator = Compat.StateObject(wrappedValue: navigator)
  }
}
