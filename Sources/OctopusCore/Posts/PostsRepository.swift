        ...
        do {
            ...
            if let error = error as? RemoteClientError {
                if case .notFound = error {
                    try? await postsDatabase.delete(postId: uuid)
...
//        }
        ...
        do {
            ...
            if let imageData = post.imageData {
                post.imageData = ImageResizer.resizeIfNeeded(imageData: imageData)
            }
            ...
            case let .success(content):
                ...
                return (Post(storablePost: finalPost, commentFeedsStore: commentFeedsStore), post.imageData)
    ...
    public func deletePost(postId: String) async throws(AuthenticatedActionError) {
        ...
        do {
            ...
            try await postsDatabase.delete(postId: postId)
            _postDeletedPublisher.send()
        } catch {
        ...
        do {
            if let likeId = post.userInteractions.userLikeId, likeId != UserInteractions.temporaryLikeId {
                do {
                    ...
                    try await postsDatabase.updateLikeId(newId: nil, postId: post.uuid)
                    ...
                    guard let error = error as? RemoteClientError,
                       case .notFound = error else {
                        ...
                        try await postsDatabase.updateLikeId(newId: likeId, postId: post.uuid)
                        throw error
                    }
